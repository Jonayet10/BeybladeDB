...
def add_battle(tournament_name, battle_date, location, player1_id, player2_id,
    ...
    try:
        ...
        print(Fore.RED + 
              f"\nInvalid date or date format '{battle_date}'. Please use 'YYYY-MM-DD' "
    ...
    try:
        ...
        if results:
            ...
            for row in results:
                ...
                # Convert is_admin flag to readable Yes/No
                admin_status = "Yes" if is_admin else "No"
                # Print each user's info in formatted rows
                print(
...
def view_user_beyblades(user_name):
    ...
    # SQL query to fetch Beyblade info for a given user. Retreives the
    # beyblade-player ID, beyblade ID, name, custom status, and beyblade
    # condition from the 'beyblades' and 'beycollection' tables for the
    # user
    query = """
    ...
    # Fetch results from query and initialize table header
    results = cursor.fetchall()
...
def view_all_battle_results_for_user(user_name):
    ...
    # Uses multiple joins to connect 'battles' table with 'users', 
    # 'beycollection' and 'beyblades' tables to gather info
    # for each player and their Beyblades. Uses WHERE clause to filter
    # results to show only battles where specified username appears as
...
def view_battle_results_for_tournament(tournament_name):
    ...
    # Uses joins to get player usernames and Beyblade details for each
    # battle
    query = """
...
def view_battle_results_for_location(location):
    ...
    # Uses joins to get usernames and Beyblade details for each player
    query = """
...
def view_beyblade_parts(beyblade_id):
    ...
    # SQL query to select all parts for a specific Beyblade, joins part IDs in
    # 'beyblades' table to fetch designated parts
    query = """
    ...
    """
    # Run query with given beyblade ID as parameter
    cursor.execute(query, (beyblade_id,))

    # Fetch all matching records (Beyblade parts)
    results = cursor.fetchall()
    headers = ["Part ID", "Part Type", "Weight (g)", "Description"]

    # Check if parts were found for the Beyblade
    if results:
        print(tabulate(results, headers=headers, tablefmt="grid"))
    else:
...
def add_user_beyblade(
    ...
    expected_types = {
        face_bolt_id: "Face Bolt",
        energy_ring_id: "Energy Ring",
        fusion_wheel_id: "Fusion Wheel",
        spin_track_id: "Spin Track",
        performance_tip_id: "Performance Tip"
    }

    # Check that each part exists and matches its expected type
    for part_id, expected_type in expected_types.items():
        cursor.execute("SELECT part_type FROM parts WHERE part_ID = %s", (part_id,))
        result = cursor.fetchone()
        if result is None:
            print(Fore.RED + f"\nError: Part ID '{part_id}' does not exist in the parts table.")
            return
        ...
        if actual_type != expected_type:
            print(Fore.RED + f"\nError: Part ID '{part_id}' is a '{actual_type}', "
                             f"but a '{expected_type}' was expected.")
            return
...
def beyblade_leaderboard():
    ...
    # SQL query to fetch Beyblade IDs, name, types, and number of wins.
    # Joins the 'battles' table with the 'beycollection' table to map
    # winning Beyblades to users, and joins with the 'beyblades' table
    # to get Beyblade info. Then groups by win count in descending 
    # order with names in alphabetical order for ties/draws
    query = """
